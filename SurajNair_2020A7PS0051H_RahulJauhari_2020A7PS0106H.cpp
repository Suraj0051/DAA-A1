...
/*!
     ...
     */ 
void decompose(DCEL &d, vector<Point> &edge)
{
...
/**
     ...
     */ 
Point next(const vector<Point> &edge, Point p)
{
    int k;
    for (int i = 0; i < edge.size(); i++)
    {
        if (edge[i].x == p.x && edge[i].y == p.y)
        {
            k = i;
            break;
        }
    }
    return edge[(k + 1) % edge.size()];
}
/**
...
void check_if_same(vector<Point>&convex_polygon,vector<Point>&edge)
{
    while(convex_polygon[0].x!=edge[0].x&&convex_polygon[0].y!=edge[0].y)
    {
        for(int i=0;i<convex_polygon.size();i++)
        {
            convex_polygon[i].x=edge[i].x;
            convex_polygon[i].y=edge[i].y;
        }
    }
}
/// @endcond
/**
     ...
     */ 
vector<vector<Point> > Decompose_polygon(vector<Point> &edge)
{
    vector<vector<Point> > result;
    vector<Point> convex_polygon, notch_list;
    // L0 fv1g; m 1.
    convex_polygon.push_back(edge[0]);   // adding the first vertex of the polygon, our algorithm starts from here
    int q = 1;
    ...
    while ( edge.size() > 3)  
    {
        q=1;
        m++;
        check_if_same(convex_polygon,edge);
        // p1 last of convex_polygon and p2 next in edge
        ...
        Point p3 = next(edge, edge[q]); // next point
        while (angle(edge[(q - 1) % edge.size()], edge[q], edge[(q + 1) % edge.size()])  && angle(edge[q], edge[(q + 1) % edge.size()], p1)  && angle(edge[(q + 1) % edge.size()], p1, p2) ) //checking if adding the next point satisfies the following 3 conditions
        {
            convex_polygon.push_back(edge[(q+1) % edge.size()]);
            convex_polygon = remove_duplicates(convex_polygon);
            q++;
            p3 = next(edge, edge[q]); //
        ...
        if (convex_polygon.size() == edge.size()) // if the given polygon is a convex polygonn itself
        {
        ...
        if(convex_polygon.size()==2) // if here are only 2 points in the list
        {
            ...
            Point temporary = edge[0];
            edge.erase(edge.begin());
            edge.push_back(temporary);
        }
        else f=0;   
        if(f==0){
            if (convex_polygon.size() != edge.size()&&convex_polygon.size()>2) // check 3.4.1
            {
                for (int i = q-1; i < edge.size(); i++)  // find the notches in the polygon
                {
                    if (notch(edge[i], edge[(i + 1) % edge.size()], edge[(i + 2) % edge.size()]))
                    {
                        // vector<float> temp = rectangele(convex_polygon);
                        // if (edge[i].x >= temp[0] && edge[i].x <= temp[2] && edge[i].y >= temp[1] && edge[i].y <= temp[3])
                            {
                                notch_list.push_back(edge[(i + 1) % edge.size()]);
                            }
                    }
                }
                vector<float> temp = rectangele(convex_polygon);
            ...
            if (convex_polygon[convex_polygon.size() - 1].x != p2.x || convex_polygon[convex_polygon.size() - 1].y != p2.y) // if the polygon obtained has more than 2 points after all the above operations then push it into result
            {
                ...
                for (int i = 0; i < convex_polygon.size(); i++)
                {
                    ...
                    {
                        if (convex_polygon[i].x == edge[j].x && convex_polygon[i].y == edge[j].y)
                        {
                            edge.erase(edge.begin() + j);q--;
                            i=0;
                            break;
                        }
                    }
                }
                edge.insert(edge.begin(), last);q++;
                edge.insert(edge.begin(), first);q++;
                edge = remove_duplicates(edge); // remove points from the original list
...
int main()
{
    ...
    vector<vector<Point> > result = Decompose_polygon(edge);
